...
function TelecallerLeadReport() {
  const { userData } = useContext(AuthContext)
  const navigate = useNavigate()

  const LIST_KEY = 'TC_LEAD_REPORT_LIST'
  ...
  const getTelecallerLeadReport = async () => {
    try {
      const response = await apiClient.get('/lead/get-telecaller-lead-report', {
        params: {
  ...
  const handleExport = async () => {
    try {
      setIsLoading('export')
      const response = await apiClient.get(`/lead/export-telecaller-lead-report`, {
        params: {
          search: filter.search,
          branchId: filter.branchId,
          roleId: filter.roleId,
      ...
      link.href = url
      link.setAttribute('download', 'TelecallerLeadReport.xlsx')
    ...
    <Container className="mt-4 container-lg p-0">
      <Card>
        <Card.Header className="d-flex justify-content-between mainBGColor text-white fw-bold py-1">
          {' '}
          <div>Telecaller Lead Report</div>
...
export default TelecallerLeadReport
