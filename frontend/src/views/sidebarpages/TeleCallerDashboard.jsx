...
const TeleCallerDashboard = () => {
  ...
  // ── Doughnut chart — Total / Pending / Completed ─────────────────────────────
  const doughnutData = {
  ...
  // ── Bar chart — Lead Status Distribution (same as Admin) ─────────────────────
  ...
  // ── Main fetch ───────────────────────────────────────────────────────────────
  const getTeleCallerDashboardData = async () => {
    try {
      ...
      const leadsResponse = await apiClient.get('/lead/get-leads', {
        ...
        params: {
          limit: 1000,
          // Backend auto-scopes to this TC's leads via JWT role — no extra param needed
  ...
  useEffect(() => {
    getTeleCallerDashboardData()
  }, [dateFilter])

  useEffect(() => {
    if (!socket) return
    const events = ['create-lead', 'update-lead', 'delete-lead', 'many-admin-to-telecaller', 'single-telecaller-to-bde']
    const handler = () => getTeleCallerDashboardData()
  ...
  return (
    <div>
      <Helmet>
        <title>TeleCaller Dashboard — Clientmark</title>
      </Helmet>
      ...
      <Container className="mt-4 container-lg p-0">
        ...
        <Row className="mb-3 align-items-center justify-content-between">
          ...
          >
            ...
            <input
              ...
              style={{ maxWidth: 140 }}
            />
            <Button className="button" onClick={() => getTeleCallerDashboardData()}>
              Apply
            </Button>
          </Col>
        </Row>
        ...
        <Row className="mb-4">
          <Col xs={12}>
            <Row className="g-3">
              {/* Doughnut — Total / Pending / Completed */}
              <Col md={6}>
